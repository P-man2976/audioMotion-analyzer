...
const deprecate = (name, alternative) =>
...
const validateFromList = (value, list, modifier = "toLowerCase") =>
	list[Math.max(0, list.indexOf(("" + value)[modifier]()))];
...
export default class AudioMotionAnalyzer {
	...
	constructor(container, options = {}) {
		...
		this._container =
			container ||
			(!this._ownCanvas && canvas.parentElement) ||
			document.body;
		...
		if (this._outNodes.length == 1) {
			for (const i of [0, 1])
				this._analyzer[i].connect(
					this._chLayout == CHANNEL_SINGLE && !i
						? this._output
						: this._merger,
					0,
					i
	...
	getOptions(ignore) {
		...
		for (const prop of Object.keys(DEFAULT_SETTINGS)) {
			if (!ignore.includes(prop)) {
				if (
					prop == "gradient" &&
					this.gradientLeft != this.gradientRight
				) {
	...
	setLedParams(params) {
		...
		this._ledParams =
			maxLeds > 0 && spaceV > 0 && spaceH >= 0
				? [maxLeds, spaceV, spaceH]
				: undefined;
		this._calcBars();
	}
	...
	toggleAnalyzer(force) {
		...
		// Start the analyzer if it was stopped and must be enabled
			...
			this._frames = 0;
			this._time = performance.now();
			this._runId = requestAnimationFrame((timestamp) =>
	...
	_calcBars() {
		...
		const {
			...
			analyzerHeight =
				(channelHeight * (isLumi || _radial ? 1 : 1 - _reflexRatio)) |
				0,
			analyzerWidth =
				canvas.width - centerX * (isDualHorizontal || _mirror != 0),
			// channelGap is **0** if isLedDisplay == true (LEDs already have spacing); **1** if canvas height is odd (windowed); **2** if it's even
			// TODO: improve this, make it configurable?
			channelGap = isDualVertical ? canvas.height - channelHeight * 2 : 0,
			initialX =
				centerX * (_mirror == -1 && !isDualHorizontal && !_radial);
		...
		const barsPush = (args) =>
			bars.push({ ...args, peak: [0, 0], hold: [0], value: [0] });
		...
		*/
		...
		if (isOctaves) {
			...
			const roundSD = (value, digits, atLeast) =>
				+value.toPrecision(
					atLeast
						? Math.max(digits, (1 + Math.log10(value)) | 0)
						: digits
				);
			...
			const bands = [0, 24, 12, 8, 6, 4, 3, 2, 1][_mode],
				bandWidth = _ansiBands
					? 10 ** (3 / (bands * 10))
					: 2 ** (1 / bands), // 10^(3/10N) or 2^(1/N)
				halfBand = bandWidth ** 0.5;

			let analyzerBars = [],
				currFreq = _ansiBands
					? 7.94328235 / (bands % 2 ? 1 : halfBand)
					: C_1;
			...
			bars.forEach(
				(bar, index) => (bar.posX = initialX + index * barWidth)
			);
			...
			unitWidth =
				analyzerWidth / (this._freqScaling(_maxFreq) - scaleMin);
			...
			for (let i = 0, posX = 0; i < bands; i++, posX += barWidth) {
				const freqLo = invFreqScaling(scaleMin + posX / unitWidth),
					...
					freqHi = invFreqScaling(
						scaleMin + (posX + barWidth) / unitWidth
					),
					[binLo, ratioLo] = calcRatio(freqLo),
					[binHi, ratioHi] = calcRatio(freqHi);
			...
			for (let i = minIndex; i <= maxIndex; i++) {
				const freq = this._binToFreq(i), // frequency represented by this index
					posX =
						initialX +
						Math.round(
							unitWidth * (this._freqScaling(freq) - scaleMin)
						); // avoid fractionary pixel values
		...
		if (isLeds) {
			...
			const dPR =
				this._pixelRatio /
				(window.devicePixelRatio > 1 && window.screen.height <= 540
					? 2
					: 1);
			...
			const customParams = this._ledParams,
				[maxLeds, spaceVRatio, spaceHRatio] =
					customParams || params[_mode];
	...
	_createScales() {
		...
		for (const item of freqLabels) {
			const [freq, label] = Array.isArray(item)
				...
				maxW =
					fontSizeX *
					(_noteLabels && !_mirror && !isDualHorizontal
						? isC
							? 1.2
							: 0.6
						: 3);
			...
			if (x >= prevX + labelWidthX / 2 && x <= analyzerWidth) {
				...
				if (
					isDualHorizontal ||
					(_mirror && (x > labelWidthX || _mirror == 1))
				)
					_scaleX.fillText(
						label,
						isDualHorizontal && _mirror != 1
				...
				prevX =
					x + Math.min(maxW, _scaleX.measureText(label).width) / 2;
			}
			...
			if (x >= prevR + labelWidthR && x < analyzerWidth - labelWidthR) {
				...
				if (
					isDualHorizontal ||
					(_mirror && (x > labelWidthR || _mirror == 1))
				)
	...
	_draw(timestamp) {
		...
		this._last =
			timestamp - (targetInterval ? frameTime % targetInterval : 0); // thanks https://stackoverflow.com/a/19772220/2370385
		this._frames++;
			...
			switch (_weightingFilter) {
				...
				case FILTER_B:
					const rB =
						(SQ12194 * f2 * freq) /
						((f2 + SQ20_6) *
							Math.sqrt(f2 + SQ158_5) *
							(f2 + SQ12194));
					return 0.17 + linearTodB(rB);

				case FILTER_C:
					const rC =
						(SQ12194 * f2) / ((f2 + SQ20_6) * (f2 + SQ12194));
		...
		const updateEnergy = (newVal) => {
			...
			if (_energy.peak > 0) {
				...
				if (_energy.hold < 0)
					_energy.peak +=
						_energy.hold / ((holdFrames * holdFrames) / 2);
			}
		...
		for (let channel = 0; channel < nChannels; channel++) {
			const { channelTop, channelBottom, analyzerBottom } =
				...
				bgColor =
					!showBgColor || (isLeds && !overlay)
						? "#000"
						: channelGradient.bgColor,
...
*/
			...
			const drawScaleY = () => {
				const scaleWidth = canvasX.height,
					...
					atEnd =
						_mirror != 1 &&
						(!isDualHorizontal || channel != _mirror);
				...
				for (let val = max; val > min; val -= incr) {
					...
					if (even) {
						const labelY =
							posY + fontSize * (posY == channelTop ? 0.8 : 0.35);
						if (atStart)
			...
			const getAngle = (x, dir = angularDirection) =>
				dir * TAU * ((x + radialOffsetX) / canvas.width) +
				this._spinAngle;
			...
			for (let barIndex = 0; barIndex < nBars; barIndex++) {
				...
				if (_mode == MODE_GRAPH) {
					...
					const nextBarAvg = barIndex
						? 0
						: (this._normalizedB(fftData[_bars[1].binLo]) *
								maxBarHeight +
								barHeight) /
						  2;
						...
						if (barIndex == 0) {
							...
							if (_mirror == -1 && !isDualHorizontal)
								...
								const prevFFTData = binLo
									? this._normalizedB(fftData[binLo - 1]) *
									  maxBarHeight
				...
				if (
					...
					if (isLeds) {
						...
						_ctx.fillRect(
							posX,
							analyzerBottom - peak * maxBarHeight,
							width,
							2
						);
			...
			if (_mode == MODE_GRAPH) {
				...
				if (fillAlpha > 0) {
					if (_radial) {
						...
						const start = isDualHorizontal
								? getAngle(analyzerWidth >> 1)
								: 0,
							end = isDualHorizontal
								? getAngle(analyzerWidth)
								: TAU;
						_ctx.moveTo(
							...radialXY(
								isDualHorizontal ? analyzerWidth >> 1 : 0,
								0
							)
						);
						_ctx.arc(
	...
	_freqToBin(freq, method = "round") {
		const max = this._analyzer[0].frequencyBinCount - 1,
			bin = Math[method](
				(freq * this.fftSize) / this.audioCtx.sampleRate
			);
